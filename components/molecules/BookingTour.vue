...
const $isOpen = computed(() => $props.target?.trip_detail?.trip_type?.toLowerCase() === 'open trip');

const $availableSchedules = computed(() => {
  return ($props.target?.schedules || []).filter(s => s.status === 'ready' && s.available_quota >= $local.quantity);
});

watch($availableSchedules, (newVal) => {
  if (!$isOpen.value) return;
  
  if (newVal.length === 1) {
    $local.schedule_id = newVal[0].id;
  } else if ($local.schedule_id && !newVal.find(s => s.id === $local.schedule_id)) {
    $local.schedule_id = null;
  }
}, { immediate: true });

onMounted(async () => {
  if ($isPrivate.value && $props.target?.client?.id) {
    try {
      const res = await $fetch(`${useRuntimeConfig()?.public.apiUrl}/clients/${$props.target.client.id}/partner-hotels`);
      if (res?.result) {
        $local.partnerHotels = res.result;
      }
    } catch (e) {
      console.error(e);
    }
...
const $tripDays = computed(() => {
  const daysDetail = $props.target?.details?.find(d => d.title?.toLowerCase() === 'days');
  return daysDetail ? Number(daysDetail.amount) : 1;
});

watch(() => $local.checkIn, (newVal) => {
  if ($isPrivate.value) {
    $local.checkOut = $dateHours(new Date($dateEdit(new Date(newVal), $tripDays.value - 1)), 12);
  }
});

watch(() => $local.includeHotel, (newVal) => {
  if (!newVal) {
    $local.hotel_id = null;
  }
});

watch(() => $local.schedule_id, (newVal) => {
  if ($isOpen.value && newVal) {
    const selected = $availableSchedules.value.find(s => s.id === newVal);
    if (selected) {
    ...
    <div v-if="$props.target">
      ...
      <section class="md:grid grid-cols-2 gap-2">
        ...
        <div class="col-span-full space-y-1">
          ...
          <atoms-text v-html="$props.target?.description || '-'"></atoms-text>
        </div>
      </section>
      ...
      <n-card size="small" title="Status">
        <section>
          ...
          <div v-if="$isOpen" class="space-y-4">
            <atoms-text caption strong class="!text-primary capitalize">Select Departure Schedule</atoms-text>
            <n-select
              v-if="$availableSchedules.length > 1"
              v-model:value="$local.schedule_id"
              :options="$availableSchedules.map(s => ({ label: moment(s.departure_date).format('DD MMM YYYY') + ' - ' + moment(s.return_date).format('DD MMM YYYY') + ' (' + s.available_quota + ' seats available)', value: s.id }))"
            ...
            <div>
              ...
              <br/>
              <n-input-number v-model:value="$local.quantity" :min="1" :max="$availableSchedules.find(s => s.id === $local.schedule_id)?.available_quota || 999" />
              <atoms-text v-if="$local.schedule_id" caption class="text-green-500 block mt-1">
          ...
          <div v-if="$isPrivate" class="grid md:grid-cols-2 md:gap-5">
            <div class="space-y-1">
              <atoms-text caption strong class="!text-primary capitalize">Start Date</atoms-text>
              <atoms-input-date
            ...
            <div class="space-y-1">
              <atoms-text caption strong class="!text-primary capitalize">End Date (Auto)</atoms-text>
              <atoms-input-date
                :value="$dateCheckOut"
                type="date"
                format="dd MMMM yyyy"
            ...
            <div v-if="$local.includeHotel" class="col-span-full space-y-2">
              ...
              <n-select
                v-model:value="$local.hotel_id"
                :options="$local.partnerHotels.map(h => ({ label: h.name + ' (+ IDR ' + $addSeparator(h.price_per_night) + '/night)', value: h.id }))"
                placeholder="Choose partner hotel"
              />
              <n-alert v-if="$local.hotel_id" type="info" :show-icon="true">
                <atoms-text caption>
                  🏨 Accommodation is calculated with <strong>1 room per 2 guests</strong> (sharing basis). For {{ $local.quantity }} pax, a total of <strong>{{ Math.ceil($local.quantity / 2) }} room(s)</strong> will be booked for <strong>{{ Math.max(1, $tripDays - 1) }} night(s)</strong>. Room assignments will be coordinated by the tour agent.
                </atoms-text>
              </n-alert>
            </div>
          ...
          <div class="bg-gradient p-5 grid md:grid-cols-2 md:gap-5 rounded-md">
            ...
            <div class="space-y-1">
              ...
              <atoms-text class="capitalize"
                >IDR
                {{
                $addSeparator(($props.target?.price || 0) * $local.quantity + ($local.hotel_id ? $local.partnerHotels.find(h => h.id === $local.hotel_id)?.price_per_night * Math.max(1, $tripDays - 1) * Math.ceil($local.quantity / 2) : 0)) || "-"
                }}</atoms-text
              >
            </div>
          </div>
        ...
        @click="
          () => {
            ...
            $onSubmitBooking({
              ...
              schedule_id: $local.schedule_id,
              hotel_id: $local.hotel_id,
