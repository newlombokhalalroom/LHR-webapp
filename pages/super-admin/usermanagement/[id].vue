<!-- pages/super-admin/usermanagement/[id].vue -->
<script setup>
...
/* ========== META ========== */
definePageMeta({
  path: "/super-admin/usermanagement/:id",
  label: "User Detail",
  title: "User Detail",
  icon: "account",
  hidden: true,
...
// ⬇️ pakai helper upload yang sudah kamu punya di contoh signup
...
/* ========== PHOTO PICKER STATE ========== */
const $uploadImage = ref(null); // ref ke input file hidden
const pickedPicture = ref(null); // { rawSource: File, source: previewUrl }
let _previewToRevoke = null;

const picturePreview = computed(() => {
  // Prioritas: gambar yang baru dipilih (preview) -> gambar existing dari userDetail
...
// ⬇️ handler photo picker
const onPickPicture = (event) => {
  const file = event?.target?.files?.[0];
  if (!file) return;

  // bersihkan preview sebelumnya biar ga memory leak
  clearPickedPicture();
  ...
  // reset supaya bisa pilih file yang sama lagi kalau perlu
  event.target.value = "";
};
...
const fetchUser = async () => {
  ...
  try {
    ...
    // reset pilihan foto kalau pindah user / refresh data
    clearPickedPicture();
  } catch (e) {
    console.error(e);
    userDetail.value = null;
    $message.error("User not found or failed to fetch data.");
  } finally {
    loading.value = false;
  }
};
...
const handleSubmitUpdate = async () => {
  if (!userDetail.value) return;
  if (!confirm("Yakin update data user ini?")) return;

  submitting.value = true;
  try {
    // ⬇️ jika user memilih foto baru, upload dulu saat submit
    ...
    const payload = {
      ...
      picture: finalPicture, // ⬅️ kirim hasil upload (string), bukan URL manual
  ...
  <atoms-container>
    ...
    <section class="flex flex-wrap items-center justify-between gap-3 mb-4">
      <div class="flex items-center gap-2">
        ...
        <atoms-heading h3 class="mb-0">User Detail</atoms-heading>
      </div>
      <div class="flex items-center gap-2">
    ...
    <n-card :segmented="{ content: true, footer: true }" class="shadow-sm rounded-2xl">
      ...
      <template #default>
        ...
        <section v-else-if="userDetail" class="grid grid-cols-1 gap-6 md:grid-cols-3">
          ...
          <n-card size="small" class="rounded-xl md:sticky md:top-4 h-fit">
            <div class="flex items-center gap-4">
              <atoms-avatar
                ...
                :zoom="false"
                sizes="84"
                :nickname="userDetail.username || 'user'"
              />
              <div class="min-w-0">
          ...
          <div class="md:col-span-2">
            <n-card size="small" class="rounded-xl">
              <template #header>
                <atoms-heading h5 class="mb-0">Edit User</atoms-heading>
              </template>
        ...
        <div v-else class="text-gray-500">User tidak ditemukan.</div>
      </template>
