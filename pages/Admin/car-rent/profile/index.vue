...
const $onFetchAnotherPolicies = async (_payload) => {
  $local.mainLoading = true;
  try {
    const _resp = await $api.get("policies?type=car rent&category=regular");
    if (_resp?.status) {
...
<template>
  <div>
    ...
    <atoms-container v-else>
      <n-button
        type="primary"
        @click="router.push({ path: '/admin/car-rent' })"
      ...
      <n-tabs v-model:value="$local.tab" type="segment" class="-translate-y-20">
        <n-tab-pane name="0" tab="Overview">
          ...
          <section v-if="$local.data?.clientFacilities?.length > 0" class="col-span-full space-y-2">
            <n-card
