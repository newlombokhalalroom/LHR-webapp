...
const $local = reactive({
  ...
  openWithdrawalConfirmation: null,
...
/* ---------------- Submit Withdraw (validasi saldo + notifikasi) ---------------- */
  ...
  try {
    ...
    if (amount > balance) {
      $notification.error({
        ...
        content: `Nominal withdraw (IDR ${$addSeparator(
          amount
    ...
    if (confirm("Are you sure to continue")) {
      ...
      $notification.success({
        title: "Success",
        content: `Your withdraw was successfully created`,
      });
...
<template>
  <div>
    ...
    <molecules-drawer
      ...
      :content="{ title: 'Withdrawal Confirmation' }"
      @closed="
        () => {
    ...
    >
      <template #default="{ data: $data, onClose: $onClose }">
        <n-space align="center" :justify="center" vertical>
          ...
          <br />
          <atoms-heading h2
            >Withdrawal {{ $local.openWithdrawalConfirmation?.status }}</atoms-heading
          >
          <br />
          <atoms-text span>
            Your withdrawal has been {{ $local.openWithdrawalConfirmation?.status }}!
          </atoms-text>
          <atoms-text span> Details of transaction are included below </atoms-text>
        </n-space>
        <n-divider />
        <n-space align="center" justify="space-between">
          <atoms-text>Total Amount Paid</atoms-text>
          <atoms-text
        ...
        <n-divider />
        <n-space align="center" justify="space-between">
          <atoms-text>Paid With</atoms-text>
          <atoms-text>{{
    ...
    <!-- Drawer Withdraw: tampilkan saldo saat ini + validasi saldo -->
    <molecules-drawer
      v-model:show="$local.openWithdrawal"
      :content="{ title: 'Withdrawal' }"
      @mounted="
        () => {
          const _fn = async () => {
      ...
      <template #default="{ data: $data }">
        <div v-if="$data">
          ...
          <n-alert v-if="$data.insufficient" type="error" title="Saldo tidak cukup" class="mt-2">
            Nominal withdraw melebihi saldo sekarang. Kurangi nominal atau isi saldo terlebih
            dahulu.
          </n-alert>

          <!-- Sisa saldo -->
          <div class="mt-2">
            <atoms-text caption strong>Remaining after withdrawal</atoms-text>
    ...
    <atoms-container>
      <br />
      <atoms-heading h2>Withdrawal</atoms-heading>
      <br />
      ...
      <n-card>
        <template #header>
          <section class="flex items-center gap-5">
            ...
            />
            <div>
              <atoms-heading h5>{{ $dataUser?.client?.name }}</atoms-heading>
              <atoms-text span
                >{{ $dataUser?.client?.phone }} ({{ $dataUser?.username || "-" }})</atoms-text
              >
            </div>
          </section>
        </template>
        ...
        <section class="space-y-2">
          <div>
            <atoms-text caption strong>Last withdrawal</atoms-text>
